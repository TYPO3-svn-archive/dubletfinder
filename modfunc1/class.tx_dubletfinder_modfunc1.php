...
class tx_dubletfinder_modfunc1 extends t3lib_extobjbase {
	...
	function reduceFeUsersDublet($email) {
		...
		if ($this->debug) {
			$output .= '<br />Kombinierte Kategorien: '.$combinedCategories.'<br />';
			$output .= 'Behalte UID '.$firstUid.', lösche alle anderen.<br />';
		}
		$output .= ': Schreibe Änderungen in fe_users.'.chr(10);
	...
	function reduceAddressDublet($email) {
		...
		$output .= ': Schreibe Änderungen in tt_address.'.chr(10);
	...
	function reduceCrossDublet($email) {
		...
		if ($firstUid) {
			...
			if ($this->debug) {
				$output .= '<br />Kombinierte Kategorien: '.$combinedCategories.'<br />';
				$output .= 'Behalte fe_users-UID '.$firstUid.', lösche alle anderen fe_users sowie tt_address.<br />';
			}
			$output .= ': Schreibe Änderungen in fe_users und tt_address.'.chr(10);
			...
			if ($this->debug) {
				$output .= '<br /><strong>WHERE-Klausel: </strong>';
			...
			if ($this->debug) {
				...
				if ($GLOBALS['TYPO3_DB']->sql_num_rows($dbResult) !== 0) {
					$output .= '<br /><strong>Dieser Datensatz wurde nicht korrekt aus tt_address gelöscht!</strong>';
				}
	...
	function createRecursivePageList() {
		...
		if ($this->debug) {
			$output .= '<p><strong>Seiten in den Direct-Mail-Gruppen:</strong> '.$this->pageList.'<br />';
			$output .= '<strong>Rekursive Seitenliste:</strong> '.$this->pageListRecursive.'</p>';
		}
